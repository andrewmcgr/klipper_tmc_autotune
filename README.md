...
| loglevel | `info` | `debug`, `info`, `off` | This parameter is used to set the level of logging to the console. Selecting `debug` will log every field and register that is being set. `info` will only log that the steppers has been tuned. `off` will disable all console logging. |
